...
contract SecondNestedCall {
    ...
    function G() public {
        ...
        assembly {
            ...
            mstore(0xd,sload(0x8002))
            if iszero(eq(sload(0x8002),add(orig_value,1))) {
                mstore(0xd,add(2200,mload(0x80)))
                revert(0xd,0x20)
            }
        }
        ...
        reqProc = bytes24("FifthhNestedCall");
        assembly {
            // First set up the input data (at memory location 0x0)
            // The call call is 0x-03
            mstore(0x0,0x03)
