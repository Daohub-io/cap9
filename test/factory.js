...
contract('Factory', function (accounts) {
    ...
    describe('.createProcedure()', async function () {
        ...
        it('should create a sample contract', async function () {
            const factory = await Factory.new();
            // Peform an ephemeral call to Factory.create
            let address = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
        ...
        it('the returned address should be deterministic and valid', async function () {
            ...
            let address1 = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            let address2 = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
        ...
        it('the returned address should not be deterministic if we make an additional transaction in between', async function () {
            const factory = await Factory.new();
            // Perform two ephemeral calls to factory.create
            const address1 = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            const tx = await factory.createProcedure("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            const address2 = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            // The addresses are valid.
            assert(web3.isAddress(address1), `The returned address (${address1}) is a valid address`);
            assert(web3.isAddress(address2), `The returned address (${address2}) is a valid address`);
            // The addresses are not null.
        ...
        it('the transaction should fail without sufficient wei', async function () {
            ...
            const address = await factory.createProcedureAndPay.call("TestAdder", 3, Adder.bytecode, {from: accounts[testAccount], value:0});
        ...
        it('the new contract should function properly', async function () {
            const factory = await Factory.new();
            let address = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            let tx = await factory.createProcedure("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            assert(web3.isAddress(address), `The returned address (${address}) is a valid address`);
            assert(!isNullAddress(address), `The returned address (${address}) is not the null address`);
        ...
        it('execute function via procedure table', async function () {
            const factory = await Factory.new();
            let address = await factory.createProcedure.call("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            let tx = await factory.createProcedure("TestAdder", Adder.bytecode, {from: accounts[testAccount], value:1000});
            assert(web3.isAddress(address), `The returned address (${address}) is a valid address`);
            ...
            let tl = await factory.executeProcedure("TestAdder", {from: accounts[testAccount]});
            let tc = await factory.executeProcedure.call("TestAdder", {from: accounts[testAccount]});
