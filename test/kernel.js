...
contract('Kernel', function (accounts) {
    ...
    describe('.listProcedures()', function () {
        ...
        it('should return existing procedure keys', async function () {
            ...
            let [err, address] = await kernel.createProcedure.call('TestAdder', Valid.Adder.bytecode, false)
            let tx1 = await kernel.createProcedure('TestAdder', Valid.Adder.bytecode, false)
        ...
        it('should return a list of procedures which can be retrieved', async function () {
            ...
            for (const proc of speccedProcedures) {
                await kernel.createProcedure(proc[0], proc[1].bytecode, false)
            }
    ...
    describe('.getProcedure()', function () {
        it('should return a non-zero address iff procedure exists', async function () {
            ...
            let [err, creationAddress] = await kernel.createProcedure.call('TestAdder', Valid.Adder.bytecode, false);
    ...
    describe('.createProcedure()', function () {
        it('should create valid procedure', async function () {
            ...
            let [err, address] = await kernel.createProcedure.call(procedureName, Valid.Adder.bytecode, false)
            let tx1 = await kernel.createProcedure(procedureName, Valid.Adder.bytecode, false)
        ...
        it('should create valid procedure (max key length)', async function () {
            ...
            const [err, address] = await kernel.createProcedure.call(name, Valid.Adder.bytecode, false)
            const tx1 = await kernel.createProcedure(name, Valid.Adder.bytecode, false)
        ...
        it('should create 2 valid procedures', async function () {
            ...
            const [err2, address2] = await kernel.createProcedure.call(name2, Valid.Adder.bytecode, false)
            const tx2 = await kernel.createProcedure(name2, Valid.Adder.bytecode, false)
        ...
        describe('should reject invalid key', function () {
            ...
            it('zero length', async function () {
                ...
                let [err, creationAddress] = await kernel.createProcedure.call('', Valid.Adder.bytecode, false)
                assert.equal(err, 1);
                assert(web3.isAddress(creationAddress), `Procedure Creation Address (${creationAddress}) is a real address`)
            ...
            it('duplicate procedure key', async function () {
                ...
                const [err1, address1] = await kernel.createProcedure.call(name, Valid.Adder.bytecode, false)
                const tx1 = await kernel.createProcedure(name, Valid.Adder.bytecode, false)

                // This is the second time the procedure is added
                const [err2, address2] = await kernel.createProcedure.call(name, Valid.Multiply.bytecode, false)
                const tx2 = await kernel.createProcedure(name, Valid.Multiply.bytecode, false)
                assert.equal(err2, 3);
    ...
    describe('.deleteProcedure()', function () {
        ...
        it('should return deleted procedure address if procedure key is valid', async function () {
            ...
            const [err1, address] = await kernel.createProcedure.call("test", Valid.Adder.bytecode, false);
            assert.equal(err1, 0);
            const tx1 = await kernel.createProcedure('test', Valid.Adder.bytecode, false)
            const code = web3.eth.getCode(address);
        ...
        it('should remove the procedure from the list on deletion', async function () {
            ...
            const [err1, address] = await kernel.createProcedure.call(procedureName, Valid.Adder.bytecode, false);
            assert.equal(err1, 0);
            const tx1 = await kernel.createProcedure(procedureName, Valid.Adder.bytecode, false)
            const code = web3.eth.getCode(address);
            const codeAsNumber = web3.toBigNumber(code);
    ...
    describe('.executeProcedure(bytes24 key, bytes payload)', function () {
        ...
        describe('should execute', function () {
            describe('Simple Procedure', function () {
                it('X() should fail', async function () {
                    const kernel = await Kernel.new();
                    const [, address] = await kernel.createProcedure.call("Simple", Invalid.Simple.bytecode, false);
                    const tx = await kernel.createProcedure("Simple", Invalid.Simple.bytecode, false);
            ...
            describe('SysCall Procedure', function () {
                it('S() should succeed when given cap', async function () {
                    const kernel = await Kernel.new();
                    const [, address] = await kernel.createProcedure.call("SysCallTest", Valid.SysCallTest.bytecode, true);
                    const tx = await kernel.createProcedure("SysCallTest", Valid.SysCallTest.bytecode, true);
                ...
                it('S() should fail when not given cap', async function () {
                    const kernel = await Kernel.new();
                    const [, address] = await kernel.createProcedure.call("SysCallTest", Valid.SysCallTest.bytecode, false);
                    const tx = await kernel.createProcedure("SysCallTest", Valid.SysCallTest.bytecode, false);
                    ...
                    assert.equal(valueX.toNumber(), 999, "S() should succeed with correct value the first time");
                    ...
                    assert.equal(value2.toNumber(), 999, "S() should succeed with correct value the second time");
                })
            })
        })
        ...
        describe('Entry Procedure', function () {
            it('should return the entry procedure address', async function () {
                ...
                const [a, address] = await kernel.createProcedure.call(procedureName, Valid.SysCallTest.bytecode, true);
                // assert.equal(a.toNumber(), 0, "S() should succeed with zero errcode the second time");
                const tx = await kernel.createProcedure(procedureName, Valid.SysCallTest.bytecode, true);
        ...
        describe('should return an error if procedure return error when', function () {
            ...
            it('throws an error', async function () {
                ...
                let [err, address] = await kernel.createProcedure.call('TestDivide', Valid.Divide.bytecode, false)
                let tx1 = await kernel.createProcedure('TestDivide', Valid.Divide.bytecode, false)
