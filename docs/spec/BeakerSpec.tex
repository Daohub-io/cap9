...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{c | c | c | c | c | X }
    \hline
    \texttt{0x} & \texttt{00} & \texttt{01 - 04} & \texttt{05 - 1f} & \texttt{1e - 20} & Description \\
    \hline
    \hline
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 00} & Proc Key & $\cdots$ & Procedure Heap \\
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 01} & \texttt{00 $\cdots$ 00} & \texttt{00 00 00} & \# of Procedures\\
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 01} & Key Index & \texttt{00 00 00} & Procedure List \\
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 02} & \texttt{00 $\cdots$ 00} & \texttt{00 00 00} & Kernel Address \\
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 03} & \texttt{00 $\cdots$ 00} & \texttt{00 00 00} & Current Procedure \\
    \texttt{0x} & \texttt{00} & \texttt{00 00 00 04} & \texttt{00 $\cdots$ 00} & \texttt{00 00 00} & Entry Procedure \\
    \hline
  \end{tabularx}
\end{table}
...
The procedure table is stored under the prefix \texttt{0x00 00 00 00 01}. The
...
\begin{verbatim}
  0x00 00 00 00 01 + 24 more bytes + 3 zero bytes
\end{verbatim}
...
\begin{table}[H]
  ...
  \begin{tabular}{l|p{0.4\textwidth}}
    ...
    \hline
    \hline
    \texttt{0x00 00 00 00 01}     & length/number of procedures
      \tabularnewline
    \texttt{0x00 00 00 00 01} + keyIndex & procedure key/name: 24 bytes
      \tabularnewline
    ~~~$\vdots$ & capabilities in series \tabularnewline
    \texttt{0x00 00 00 00 01} + n & procedure key/name for procedure $n$
      \tabularnewline
    \hline
  \end{tabular}
\end{table}
...
Procedure data is stored using the key \texttt{0x00 00 00 00 00}. This is
combined with the procedure key to produce the following:

\begin{verbatim}
  0x00 00 00 00 00 + 24 bytes procedure key + 3 bytes data offset
\end{verbatim}
